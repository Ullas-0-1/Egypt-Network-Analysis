...
top10_moran = top10(city_stats, 'avg_d_speed_mbps_local_moran', ascending=False)
top10_geary = top10(city_stats, 'avg_d_speed_mbps_local_geary', ascending=True)  # Low is good
top10_getis = top10(city_stats, 'avg_d_speed_mbps_local_getis', ascending=False)
...
axs[0].barh(top10_moran['city'], top10_moran['avg_d_speed_mbps_local_moran'], color='seagreen')
axs[0].set_title("Top 10 Cities - Local Moran's I ")
axs[0].invert_yaxis()

axs[1].barh(top10_geary['city'], top10_geary['avg_d_speed_mbps_local_geary'], color='tomato')
axs[1].set_title("Top 10 Cities - Local Geary's C ")
axs[1].invert_yaxis()

axs[2].barh(top10_getis['city'], top10_getis['avg_d_speed_mbps_local_getis'], color='steelblue')
axs[2].set_title("Top 10 Cities - Local Getis-Ord G* ")
axs[2].invert_yaxis()

plt.tight_layout()
plt.savefig("top_10_cities_local_stats_download.png", dpi=300)
plt.show()

for i, row in top10_moran.iterrows():
    print(f"City: {row['city']}, Local Moran's I: {row['avg_d_speed_mbps_local_moran']}")

print("-" * 50)
for i, row in top10_geary.iterrows():
    print(f"City: {row['city']}, Local Geary's C: {row['avg_d_speed_mbps_local_geary']}")
print("-" * 50)
for i, row in top10_getis.iterrows():
    print(f"City: {row['city']}, Local Getis-Ord G*: {row['avg_d_speed_mbps_local_getis']}")
print("-" * 50)
